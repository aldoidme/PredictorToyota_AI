...
def clean_new_prices_data(df: pd.DataFrame) -> pd.DataFrame:
    """Limpia y normaliza precios nuevos."""
    ...
    data = drop_missing(data, ["model_id", "price_pen", "currency", "as_of"], "precios nuevos")
    data = data[data["currency"] == "PEN"]
    data = drop_missing(data, ["model_id", "price_pen"], "precios nuevos")
