document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateModel(modelId, label) {
    ...
    const stockAction = compareLabel === "Conviene nuevo" ? "comprar" : compareLabel === "Conviene usado" ? "mantener" : "mantener";
  ...
  chips.forEach((chip) => {
    chip.addEventListener("click", () => {
      ...
      updateModel(chip.dataset.model, chip.dataset.label);
    });
  });

  const defaultChip = chips[0];
  if (defaultChip) {
    updateModel(defaultChip.dataset.model, defaultChip.dataset.label);
  }
});
